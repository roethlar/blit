...
$bin = Join-Path $root "target\$cfg\robosync.exe"
if (-not (Test-Path $bin)) { throw "Binary not found at $bin" }

$tmp = Join-Path $env:TEMP ("robosync-smoke-" + [Guid]::NewGuid().Guid)
...
try {
  & $bin $src ("robosync://127.0.0.1:$port/") --mir | Out-Null
...
Start-Sleep -Seconds 2
try {
  & $bin ("robosync://127.0.0.1:$port2/") $pull --mir | Out-Null
