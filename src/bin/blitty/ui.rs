...
pub fn draw(f: &mut Frame, app: &AppState) {
    ...
    draw_pane(
        ...
        app.loading_pane == Some(Focus::Left),
        true,
    );
    draw_pane(
        f,
    ...
    // Console output pane (scrollable)
    let area = chunks[2];
    let visible = area.height.saturating_sub(1) as usize; // leave one line for padding
    let total = app.log.len();
    let off = app.log_scroll.min(total);
    let start = total.saturating_sub(visible + off);
    ...
    let logp = Paragraph::new(lines).block(
        Block::default()
            .borders(Borders::TOP)
            .title(Span::styled(
                " Console Output ",
    ...
    let p = Paragraph::new(status_lines).block(
        Block::default()
            .borders(Borders::TOP)
            .border_style(ratatui::style::Style::default().fg(Theme::COMMENT()))
            .style(ratatui::style::Style::default().bg(Theme::BG())),
    ...
    if get_show_help(app) {
        ...
        let lines = vec![
            ...
            Line::from(""),
            Line::from("  Appearance Theme tab for visual options"),
            Line::from(Span::styled(
                "General:",
                ratatui::style::Style::default()
                    .fg(Theme::CYAN())
    ...
    unsafe {
        CURRENT_LOGICAL_INDEX = usize::MAX;
    }
    ...
    if app.ui_mode == super::app::UiMode::Options {
        ...
        match app.options_tab {
            0 => {
                ...
                items.push((
                    format!("Mode: {} (Left/Right/Enter to cycle)", mode_label),
                    "".into(),
                    50,
                ));
                ...
                items.push((
                    "Include empty directories".into(),
                    on(app.options.include_empty).into(),
                    2,
                ));
                items.push((
                    "Update only (no delete)".into(),
                    on(app.options.update).into(),
                    3,
                ));
                items.push((
                    "Ludicrous speed (safe-fast)".into(),
                    on(app.options.ludicrous_speed).into(),
                    7,
                ));
                items.push((
                    "  Boosts workers/chunk sizes; disables verify and resume".into(),
                    "".into(),
                    usize::MAX,
                ));
                items.push((
                    "  TLS stays enabled; use Unsafe Mode to disable".into(),
                    "".into(),
                    usize::MAX,
                ));
            ...
            1 => {
                // Safety
                items.push((
                    "Compare by checksum".into(),
                    on(app.options.checksum).into(),
                    4,
                ));
                items.push((
                    "Skip post-verify".into(),
                    on(app.options.no_verify).into(),
                    5,
                ));
                items.push((
                    "Disable resume (not recommended)".into(),
                    on(app.options.no_restart).into(),
                    6,
                ));
            ...
            2 => {
                // Performance
                let thr = if app.options.threads == 0 {
                    "auto".to_string()
                } else {
                    app.options.threads.to_string()
                };
                let nws = if app.options.net_workers == 0 {
                    "auto".to_string()
                } else {
                    app.options.net_workers.to_string()
                };
                let ncm = if app.options.net_chunk_mb == 0 {
                    "auto".to_string()
                } else {
                    app.options.net_chunk_mb.to_string()
                };
                items.push((
                    format!("Threads (-t): {} (Left/Right adjust, Backspace auto)", thr),
                    "".into(),
                    100,
                ));
                items.push((
                    format!("Net workers: {} (Left/Right adjust, Backspace auto)", nws),
                    "".into(),
                    101,
                ));
                items.push((
                    format!("Net chunk MB: {} (Left/Right adjust, Backspace auto)", ncm),
                    "".into(),
                    102,
                ));
                // keep cursor bounds enforced in event handler
            ...
            3 => {
                ...
                items.push((
                    format!(
                        "Exclude files (xf): {} (Enter=add, Del=remove selected)",
                        app.options.exclude_files.len()
                    ),
                    "".into(),
                    220,
                ));
                ...
                items.push((
                    format!(
                        "Exclude dirs (xd): {} (Enter=add, Del=remove selected)",
                        app.options.exclude_dirs.len()
                    ),
                    "".into(),
                    221,
                ));
            ...
            4 => {
                ...
                items.push((
                    "Copy symlinks as links (do not follow)".into(),
                    on(app.options.sl).into(),
                    200,
                ));
                #[cfg(windows)]
                {
                    items.push((
                        "Copy junctions as junctions (Windows)".into(),
                        on(app.options.sj).into(),
                        201,
                    ));
                    items.push((
                        "Windows symlink privileges required (Dev Mode/admin)".into(),
                        "".into(),
                        usize::MAX,
                    ));
                }
                items.push((
                    "Exclude all symlinks".into(),
                    on(app.options.xj).into(),
                    202,
                ));
                items.push((
                    "Exclude directory symlinks".into(),
                    on(app.options.xjd).into(),
                    203,
                ));
                items.push((
                    "Exclude file symlinks".into(),
                    on(app.options.xjf).into(),
                    204,
                ));
                // keep cursor bounds enforced in event handler
            }
            5 => {
                ...
                items.push((
                    format!("Log file: {} (Enter=set, Del=clear)", path),
                    "".into(),
                    230,
                ));
                // keep cursor bounds enforced in event handler
            ...
            6 => {
                ...
                items.push((
                    format!("Recent hosts: {}", app.options.recent_hosts.len()),
                    "".into(),
                    usize::MAX,
                ));
                for (i, h) in app.options.recent_hosts.iter().take(5).enumerate() {
                    items.push((
                        format!("- {}:{} (Enter=connect)", h.host, h.port),
                        "".into(),
                        300 + i,
                    ));
                }
                ...
                for (i, d) in app.discovered.iter().take(8).enumerate() {
                    let label = format!("• {} → {}:{} (Enter=connect)", d.name, d.host, d.port);
                    items.push((label, "".into(), 400 + i));
                }
                // keep cursor bounds enforced in event handler
            }
            _ => {}
        ...
        for (idx, (label, val, logical)) in items.iter().enumerate() {
            let marker = if idx == app.options_cursor {
                if is_ascii_mode() {
                    ">"
                } else {
                    "➤"
        ...
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            "Ctrl+←/→ switch tab • F4 quick theme • Esc save switch tab • Esc save & close close",
            ratatui::style::Style::default().fg(Theme::COMMENT()),
        )));
    ...
    if app.ui_mode == super::app::UiMode::ConfirmTyped
        || app.ui_mode == super::app::UiMode::ConfirmTransfer
    {
        ...
        if let Some(req) = &app.confirm_required_input {
            lines.push(Line::from(Span::styled(
                format!("Type '{}' to confirm:", req),
                ratatui::style::Style::default().fg(Theme::PINK()),
            )));
    ...
    if app.ui_mode == super::app::UiMode::NewFolderInput {
        ...
        let location = match &app.right {
            ...
            Pane::Remote {
                host, port, cwd, ..
            } => format!("in: {}:{}{}", host, port, cwd.display()),
        };
...
pub fn current_options_logical_index() -> usize {
    unsafe { CURRENT_LOGICAL_INDEX }
}
...
fn draw_header(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let mut header_text = vec![
        Line::from(vec![
            ...
            Span::styled(
                &src_display,
                ratatui::style::Style::default().fg(if app.src.is_some() {
                    Theme::GREEN()
                } else {
                    Theme::RED()
                }),
            ...
            Span::raw("TAR:"),
            Span::styled(
                if app.tar_small { check } else { cross },
                if app.tar_small {
                    ratatui::style::Style::default().fg(Theme::GREEN())
                } else {
                    ratatui::style::Style::default().fg(Theme::COMMENT())
            ...
            Span::raw(" Delta:"),
            Span::styled(
                if app.delta_large { check } else { cross },
                if app.delta_large {
                    ratatui::style::Style::default().fg(Theme::GREEN())
                } else {
                    ratatui::style::Style::default().fg(Theme::COMMENT())
            ...
            Span::raw(" Empty:"),
            Span::styled(
                if app.include_empty { check } else { cross },
                if app.include_empty {
                    ratatui::style::Style::default().fg(Theme::GREEN())
                } else {
                    ratatui::style::Style::default().fg(Theme::COMMENT())
                },
            ...
            Span::raw(" Checksum:"),
            Span::styled(
                if app.checksum { check } else { cross },
                if app.checksum {
                    ratatui::style::Style::default().fg(Theme::GREEN())
                } else {
                    ratatui::style::Style::default().fg(Theme::COMMENT())
        ...
        Line::from(vec![
            ...
            Span::styled(
                &dest_display,
                ratatui::style::Style::default().fg(if app.dest.is_some() {
                    Theme::GREEN()
                } else {
                    Theme::RED()
                }),
...
fn draw_pane(
    ...
    let (title, entries, selected) = match pane {
        ...
        Pane::Remote {
            ...
            let label = if is_source { "Source" } else { "Target" };
            let title = format!(" {} {} {} {} ", icon, label, host_port, breadcrumb);
            (title, entries, *selected)
        }
    };
...
pub fn process_server_input(app: &mut super::app::AppState) {
    ...
    let (host, port) = if let Some(colon_pos) = input.rfind(':') {
        ...
        if host_part.trim().is_empty() {
            app.status = "Host cannot be empty".to_string();
            return;
        }
        if host_part.contains(' ') {
            app.status = "Host must not contain spaces".to_string();
            return;
        }
        match port_part.parse::<u16>() {
            Ok(p) if p > 0 => (host_part.to_string(), p),
            _ => {
...
fn get_show_help(app: &AppState) -> bool {
    app.help_visible
}
...
pub fn go_up(app: &mut super::app::AppState) {
    ...
    let (_is_remote, host_opt, port_opt, parent_path) = {
        ...
        match pane {
            Pane::Local { cwd, .. } => (false, None, None, cwd.parent().map(|p| p.to_path_buf())),
            Pane::Remote {
                cwd, host, port, ..
            } => (
                true,
